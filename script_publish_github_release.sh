...
curl -o /dev/null -sH "$PUBLISH_AUTH_HEADER" $PUBLISH_REPO_URL || { stageEcho "Connection error";  exit 1; }

stageEcho "Create a release $PUBLISH_COMMIT_TAG"

curl \
  -X POST \
  -H "$PUBLISH_AUTH_HEADER" \
  $PUBLISH_RELEASE_URL \
  -d "{\"tag_name\":\"$PUBLISH_COMMIT_TAG\"}" || { stageEcho "Release creation error $PUBLISH_COMMIT_TAG";  exit 1; }
...
[ "$id" ] || { stageEcho "Error getting release ID: $PUBLISH_COMMIT_TAG"; echo "$response" | awk 'length($0)<100' >&2; exit 1; }

for i in "${files[@]}"
do
  filename=$i
  stageEcho "Unloading $i"
